                ...
                <div className="flex items-center gap-5">
                    ...
                    <div>
                        <h2 className="font-black italic uppercase text-4xl text-slate-900 tracking-tighter leading-none">
                            ID <span className="text-transparent bg-clip-text bg-gradient-to-r from-green-600 to-emerald-600">BILLING</span>
                        </h2>
                        <p className="text-xs font-bold text-slate-400 uppercase tracking-[0.2em] mt-1">Akumulasi Tagihan & Export Retribusi</p>
                    </div>
                </div>
            ...
            {billingData.length > 0 && (
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                    <div className="bg-gradient-to-br from-white via-slate-50/50 to-blue-50/30 p-4 rounded-xl shadow-md border border-slate-200/50 backdrop-blur-sm">
                        <p className="text-[9px] font-black uppercase text-slate-500 mb-1">Total Petugas</p>
                    ...
                    <div className="bg-gradient-to-br from-white via-green-50/30 to-slate-50/50 p-4 rounded-xl shadow-md border border-slate-200/50 backdrop-blur-sm">
                        ...
                        <p className="text-lg font-black text-green-600">Rp {totalTagihan.toLocaleString('id-ID')}</p>
                    </div>
                    ...
                    <div className="bg-gradient-to-br from-white via-purple-50/30 to-slate-50/50 p-4 rounded-xl shadow-md border border-slate-200/50 backdrop-blur-sm">
                        ...
                        <p className="text-lg font-black text-slate-900">Rp {Math.round(totalTagihan / filteredBillingData.length).toLocaleString('id-ID')}</p>
                    </div>
                </div>
            )}
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead className="sticky top-0">
                            <tr className="bg-gradient-to-r from-slate-900 to-slate-800">
                                <th className="px-6 py-5 text-center text-[10px] font-black text-slate-300 uppercase tracking-[0.15em]">NO</th>
                                <th className="px-8 py-5 min-w-[200px] text-[10px] font-black text-slate-300 uppercase tracking-[0.15em]">ARMADA</th>
                                <th className="px-6 py-5 min-w-[130px] text-[10px] font-black text-slate-300 uppercase tracking-[0.15em]">MANDOR</th>
                                <th className="px-6 py-5 text-right min-w-[120px] text-[10px] font-black text-slate-300 uppercase tracking-[0.15em]">TARIF</th>
                                <th className="px-6 py-5 text-center min-w-[160px] text-[10px] font-black text-slate-300 uppercase tracking-[0.15em]">JUMLAH</th>
                                <th className="px-6 py-5 text-right min-w-[150px] text-[10px] font-black text-slate-300 uppercase tracking-[0.15em]">TOTAL TAGIHAN</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-100">
                            ...
                            ) : filteredBillingData.length > 0 ? filteredBillingData.map((item, index) => (
                                <tr key={index} className="hover:bg-gradient-to-r hover:from-green-50/50 hover:to-emerald-50/50 transition-all group border-l-4 border-l-transparent hover:border-l-green-300">
                                    <td className="px-6 py-5 text-center font-black text-slate-400 text-sm">{index + 1}</td>
                                ...
                                <tr>
                                    <td colSpan="6" className="px-6 py-40 text-center">
                                        <div className="inline-block">
                                            ...
                                            <div className="font-black text-slate-300 uppercase text-2xl tracking-[0.4em]">Tidak Ada Data</div>
                                            <p className="text-xs text-slate-400 mt-2">Coba ubah filter untuk melihat data billing</p>
                                        </div>
                                    </td>
                                </tr>
                            )}

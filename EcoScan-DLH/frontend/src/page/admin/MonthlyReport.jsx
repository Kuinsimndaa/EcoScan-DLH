...
import { FileSpreadsheet, Calendar, Download, Clock, Filter, Trash2, BarChart3 } from 'lucide-react';
                ...
                <div className="flex items-center gap-5">
                    ...
                    <div>
                        <h2 className="font-black italic uppercase text-4xl text-slate-900 tracking-tighter leading-none">
                            REKAPAN <span className="text-transparent bg-clip-text bg-gradient-to-r from-green-600 to-emerald-600">LAPORAN</span>
                        </h2>
                        <p className="text-xs font-bold text-slate-400 uppercase tracking-[0.2em] mt-1">Filter Harian & Bulanan dengan Export Data</p>
                    </div>
                </div>
            ...
            {laporan.length > 0 && (
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                    ...
                    <div className="bg-gradient-to-br from-white via-green-50/30 to-slate-50/50 p-4 rounded-xl shadow-md border border-slate-200/50 backdrop-blur-sm">
                        ...
                        <p className="text-xl font-black text-green-600">Rp {totalTarif.toLocaleString('id-ID')}</p>
                    </div>
                    <div className="bg-gradient-to-br from-white via-amber-50/30 to-slate-50/50 p-4 rounded-xl shadow-md border border-slate-200/50 backdrop-blur-sm">
                        ...
                        <p className="text-xl font-black text-slate-900">Rp {Math.round(totalTarif / laporan.length).toLocaleString('id-ID')}</p>
                    </div>
                    <div className="bg-gradient-to-br from-white via-purple-50/30 to-slate-50/50 p-4 rounded-xl shadow-md border border-slate-200/50 backdrop-blur-sm">
                        <p className="text-[9px] font-black uppercase text-slate-500 mb-1">Armada Unik</p>
                        <p className="text-3xl font-black text-blue-600">{new Set(laporan.map(x => x.namaPengendara)).size}</p>
                    </div>
                </div>
            )}
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead className="sticky top-0">
                            <tr className="bg-gradient-to-r from-slate-900 to-slate-800">
                                <th className="px-6 py-5 text-[10px] font-black text-slate-300 uppercase tracking-[0.15em]"><div className="flex items-center gap-2"><Clock size={14}/> TANGGAL / WAKTU</div></th>
                                <th className="px-6 py-5 text-[10px] font-black text-slate-300 uppercase tracking-[0.15em]">NAMA PETUGAS</th>
                                <th className="px-6 py-5 text-[10px] font-black text-slate-300 uppercase tracking-[0.15em]">MANDOR</th>
                                <th className="px-6 py-5 text-[10px] font-black text-slate-300 uppercase tracking-[0.15em] text-center">TOTAL MASUK</th>
                                <th className="px-6 py-5 text-[10px] font-black text-slate-300 uppercase tracking-[0.15em] text-right">TARIF</th>
                                <th className="px-6 py-5 text-[10px] font-black text-slate-300 uppercase tracking-[0.15em] text-center">AKSI</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-100">
                            ...
                            ) : laporan.length > 0 ? laporan.map((item, idx) => (
                                <tr key={item.id} className={`hover:bg-gradient-to-r hover:from-green-50/50 hover:to-blue-50/50 transition-all group border-l-4 ${idx % 2 === 0 ? 'border-l-transparent' : 'border-l-green-200/30'}`}>
                                    <td className="px-6 py-5">
                                        <div className="font-black text-slate-900 text-sm italic">{formatTanggalIndonesia(item.tanggalLengkap)}</div>
                                        <div className="text-[9px] font-bold text-slate-500 uppercase mt-0.5 px-2 py-1 bg-slate-100 group-hover:bg-green-100 rounded transition-colors w-fit">{item.waktu} WIB</div>
                                    </td>
                                    <td className="px-6 py-5">
                                ...
                                <tr>
                                    <td colSpan="6" className="px-6 py-40 text-center">
                                        <div className="inline-block">
                                            ...
                                            <div className="font-black text-slate-300 uppercase text-2xl tracking-[0.4em]">Data Tidak Ditemukan</div>
                                            <p className="text-xs text-slate-400 mt-2">Coba ubah filter untuk melihat data</p>
                                        </div>
                                    </td>
                                </tr>
                            )}

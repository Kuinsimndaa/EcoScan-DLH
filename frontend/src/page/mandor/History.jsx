    ...
    useEffect(() => {
        const fetchMyHistory = async () => {
            try {
                ...
                if (result.success) {
                    const myData = result.data.filter(item => item.id_user === user.id);
                    setHistory(myData);
                }
            } catch (error) { console.error("Gagal ambil history"); }
        };
        fetchMyHistory();
    }, [user.id]);
        ...
        <div className="space-y-10">
            <header>
                <div className="pl-4 border-l-8 border-green-500">
                    ...
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.4em] mt-4">Rekap Kerja Anda Hari Ini</p>
                </div>
            </header>
            ...
            <div className="bg-white rounded-[4rem] shadow-2xl border border-slate-100 overflow-hidden">
                <div className="p-10 bg-slate-900 text-white flex justify-between items-center text-sm font-black tracking-widest uppercase">
                    ...
                    <span className="bg-green-500 text-slate-900 px-6 py-2 rounded-full text-[10px]">{history.length} Scan</span>
                </div>
                        ...
                        <tbody className="text-xs font-bold text-slate-600 italic">
                            {history.length > 0 ? history.map((item, i) => (
                                <tr key={i} className="border-b border-slate-50 hover:bg-slate-50 transition-all group">
                                    <td className="p-6 text-slate-400 font-medium">{new Date(item.tanggal).toLocaleTimeString()}</td>
                                    <td className="p-6 uppercase text-slate-900 font-black not-italic tracking-tight">{item.nama_pengendara}</td>
                                ...
                                <tr><td colSpan="4" className="p-20 text-center text-slate-300 uppercase italic">Belum ada data scan</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>

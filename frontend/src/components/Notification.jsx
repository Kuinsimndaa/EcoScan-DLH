...
const Notification = ({ type = 'success', title, message, duration = 4000, onClose, playSound = false }) => {
    ...
    return (
        <div className="fixed top-6 right-6 z-50 animate-in slide-in-from-top fade-in duration-300">
            <div className={`${bgColor} border-2 rounded-[1.5rem] p-5 shadow-2xl max-w-md min-w-[350px] backdrop-blur-sm`}>
                <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                        <h3 className={`font-black uppercase text-lg ${titleColor} tracking-tight`}>
                            {title}
                        </h3>
                        <p className={`text-sm font-medium mt-1 ${messageColor}`}>
                            {message}
                        </p>
                    </div>
                ...
                {/* Progress Bar */}
                <div className={`mt-4 h-1 rounded-full overflow-hidden ${type === 'success' ? 'bg-green-200' : 'bg-red-200'}`}>
                    <div 

      ...
      <div className="max-w-7xl mx-auto flex justify-between items-center">
        ...
        <Link to="/" className="flex items-center gap-3 group">
          <motion.img 
            ...
            onError={(e) => { e.target.src = "https://upload.wikimedia.org/wikipedia/commons/0/0a/Lambang_Kabupaten_Tegal.png" }}
          />
          <div>
            <h1 className="text-lg font-black text-slate-900 leading-none tracking-tight">
              EcoScan <span className="text-green-600">DLH</span>
            </h1>
            <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Kab. Tegal</p>
          </div>
        </Link>
        ...
        {user?.role === 'admin' && (
          <div className="hidden md:flex items-center gap-1">
            ...
            <NavLink to="/admin/add-armada" active={isActive('/admin/add-armada')}>Armada</NavLink>
            <NavLink to="/admin/report" active={isActive('/admin/report')}>Laporan</NavLink>
          </div>
        )}
        ...
        <div className="flex items-center gap-4">
          <div className="text-right hidden sm:block border-r pr-4 border-slate-200">
            <p className="text-xs font-black text-slate-800 leading-none">{user?.nama || 'Pengguna'}</p>
          ...
          >
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 group-active:scale-75 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor">
